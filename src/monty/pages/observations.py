...
def render_sidebar():
    with st.sidebar:
        ...
        st.page_link("src/monty/pages/students.py", label="👥 Students", icon="👥")
        st.page_link("src/monty/pages/schedule.py", label="📅 Schedule", icon="📅")
        st.page_link("src/monty/pages/observations.py", label="👁️ Observations", icon="👁️")
        st.page_link("src/monty/pages/reports.py", label="📊 Reports", icon="📊")
        st.page_link("src/monty/pages/materials.py", label="📦 Materials", icon="📦")
        st.page_link("src/monty/pages/daily_tracking.py", label="📝 Daily Tracking", icon="📝")
        st.page_link("src/monty/pages/settings.py", label="⚙️ Settings", icon="⚙️")
        ...
        with st.container(border=True):
            ...
            with col_delete:
                if st.button("Delete", key=f"delete_obs_{obs['id']}", use_container_width=True):
                    ...
                    st.success(f"Deleted observation for {obs['student']}")
                    st.rerun()
    ...
    if "edit_observation" in st.session_state:
        ...
        with col_save:
            if st.button("Update Observation", use_container_width=True):
                for o in st.session_state.observations:
                    if o["id"] == obs["id"]:
                        o["student"] = student
                        o["date"] = date.strftime("%Y-%m-%d")
                        o["area"] = area
                        o["skills"] = skills
                        o["notes"] = notes
                        break
                del st.session_state.edit_observation
                st.success("Observation updated successfully!")
                st.rerun()
        ...
        if st.button("Save Observation", use_container_width=True):
            if not student:
                ...
                new_id = max([o["id"] for o in st.session_state.observations], default=0) + 1
                new_observation = {
                    "id": new_id,
                    "student": student,
                ...
                st.session_state.observations.append(new_observation)
                st.success(f"Observation for {student} saved successfully!")
                st.rerun()
